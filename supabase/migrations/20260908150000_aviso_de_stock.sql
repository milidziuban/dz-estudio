-- ============================================================
-- El pedido sin stock dice cuál y cuántos quedan — 08/09/2026
--
-- Desde la migración de esta mañana la base rechaza el pedido que supera el
-- stock, pero el checkout mostraba para todo el mismo error genérico
-- ("reintentá en un momento"), que además es un mal consejo: reintentar no
-- crea stock.
--
-- El error ahora viaja con dos campos que PostgREST le pasa tal cual al
-- navegador: `hint` dice de qué se trata (`sin-stock`, `fuera-de-venta`) y
-- `detail` trae el producto y cuántos quedan en JSON. Así el texto que lee la
-- clienta lo escribe src/pages/Checkout.tsx y no queda atado a la redacción
-- de un mensaje de Postgres.
-- ============================================================
